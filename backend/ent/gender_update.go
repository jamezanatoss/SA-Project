...
func (gu *GenderUpdate) AddGenderIDIDs(ids ...int) *GenderUpdate {
	gu.mutation.AddGenderIDIDs(ids...)
	return gu
}

// AddGenderID adds the Gender_ID edges to User.
func (gu *GenderUpdate) AddGenderID(u ...*User) *GenderUpdate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
	return gu.AddGenderIDIDs(ids...)
}
...
func (gu *GenderUpdate) RemoveGenderIDIDs(ids ...int) *GenderUpdate {
	gu.mutation.RemoveGenderIDIDs(ids...)
	return gu
}

// RemoveGenderID removes Gender_ID edges to User.
func (gu *GenderUpdate) RemoveGenderID(u ...*User) *GenderUpdate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
	return gu.RemoveGenderIDIDs(ids...)
}
...
func (gu *GenderUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if nodes := gu.mutation.RemovedGenderIDIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   gender.GenderIDTable,
			Columns: []string{gender.GenderIDColumn},
			Bidi:    false,
	...
	if nodes := gu.mutation.GenderIDIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   gender.GenderIDTable,
...
// AddGenderIDIDs adds the Gender_ID edge to User by ids.
func (guo *GenderUpdateOne) AddGenderIDIDs(ids ...int) *GenderUpdateOne {
	guo.mutation.AddGenderIDIDs(ids...)
	return guo
}

// AddGenderID adds the Gender_ID edges to User.
func (guo *GenderUpdateOne) AddGenderID(u ...*User) *GenderUpdateOne {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
	return guo.AddGenderIDIDs(ids...)
}
...
// RemoveGenderIDIDs removes the Gender_ID edge to User by ids.
func (guo *GenderUpdateOne) RemoveGenderIDIDs(ids ...int) *GenderUpdateOne {
	guo.mutation.RemoveGenderIDIDs(ids...)
	return guo
}

// RemoveGenderID removes Gender_ID edges to User.
func (guo *GenderUpdateOne) RemoveGenderID(u ...*User) *GenderUpdateOne {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
	return guo.RemoveGenderIDIDs(ids...)
}
...
func (guo *GenderUpdateOne) sqlSave(ctx context.Context) (ge *Gender, err error) {
	...
	if nodes := guo.mutation.RemovedGenderIDIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   gender.GenderIDTable,
	...
	if nodes := guo.mutation.GenderIDIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   gender.GenderIDTable,
