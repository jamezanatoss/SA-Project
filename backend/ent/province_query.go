...
func (pq *ProvinceQuery) QueryProvinceID() *UserQuery {
...
func (pq *ProvinceQuery) WithProvinceID(opts ...func(*UserQuery)) *ProvinceQuery {
	query := &UserQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withProvinceID = query
	return pq
}
...
//		GroupBy(province.FieldProvinceName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Province.Query().
//		Select(province.FieldProvinceName).
//		Scan(ctx, &v)
//
...
func (pq *ProvinceQuery) sqlAll(ctx context.Context) ([]*Province, error) {
	...
	if query := pq.withProvinceID; query != nil {
		...
		query.Where(predicate.User(func(s *sql.Selector) {
			s.Where(sql.InValues(province.ProvinceIDColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.province_province_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "province_province_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "province_province_id" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.ProvinceID = append(node.Edges.ProvinceID, n)
		}
	}
