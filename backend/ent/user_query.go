...
type UserQuery struct {
	...
	// eager-loading edges.
	withGenderID   *GenderQuery
	withUserTypeID *UserTypeQuery
	withProvinceID *ProvinceQuery
	withFKs        bool
...
// QueryGenderID chains the current query on the Gender_ID edge.
func (uq *UserQuery) QueryGenderID() *GenderQuery {
...
func (uq *UserQuery) QueryUserTypeID() *UserTypeQuery {
...
// QueryProvinceID chains the current query on the Province_ID edge.
func (uq *UserQuery) QueryProvinceID() *ProvinceQuery {
	query := &ProvinceQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
//  WithGenderID tells the query-builder to eager-loads the nodes that are connected to
// the "Gender_ID" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithGenderID(opts ...func(*GenderQuery)) *UserQuery {
	query := &GenderQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withGenderID = query
	return uq
}
...
func (uq *UserQuery) WithUserTypeID(opts ...func(*UserTypeQuery)) *UserQuery {
	query := &UserTypeQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withUserTypeID = query
	return uq
}

//  WithProvinceID tells the query-builder to eager-loads the nodes that are connected to
// the "Province_ID" edge. The optional arguments used to configure the query builder of the edge.
func (uq *UserQuery) WithProvinceID(opts ...func(*ProvinceQuery)) *UserQuery {
	query := &ProvinceQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withProvinceID = query
	return uq
}
...
//
//	client.User.Query().
//		GroupBy(user.FieldIdentityCard).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		loadedTypes = [3]bool{
			uq.withGenderID != nil,
			uq.withUserTypeID != nil,
			uq.withProvinceID != nil,
		}
	)
	if uq.withGenderID != nil || uq.withUserTypeID != nil || uq.withProvinceID != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := uq.withGenderID; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*User)
		for i := range nodes {
			if fk := nodes[i].gender_gender_id; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "gender_gender_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.GenderID = n
			}
		}
	}

	if query := uq.withUserTypeID; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*User)
		for i := range nodes {
			if fk := nodes[i].user_type_user_type_id; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_type_user_type_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.UserTypeID = n
			}
		}
	}

	if query := uq.withProvinceID; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*User)
		for i := range nodes {
			if fk := nodes[i].province_province_id; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "province_province_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.ProvinceID = n
			}
		}
	}
