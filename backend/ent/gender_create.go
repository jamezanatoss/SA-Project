...
func (gc *GenderCreate) AddGenderIDIDs(ids ...int) *GenderCreate {
	gc.mutation.AddGenderIDIDs(ids...)
	return gc
}

// AddGenderID adds the Gender_ID edges to User.
func (gc *GenderCreate) AddGenderID(u ...*User) *GenderCreate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
	return gc.AddGenderIDIDs(ids...)
}
...
func (gc *GenderCreate) createSpec() (*Gender, *sqlgraph.CreateSpec) {
	...
	if nodes := gc.mutation.GenderIDIDs(); len(nodes) > 0 {
