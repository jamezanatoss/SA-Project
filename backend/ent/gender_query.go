...
func (gq *GenderQuery) QueryGenderID() *UserQuery {
...
func (gq *GenderQuery) WithGenderID(opts ...func(*UserQuery)) *GenderQuery {
	query := &UserQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
	gq.withGenderID = query
	return gq
}
...
func (gq *GenderQuery) sqlAll(ctx context.Context) ([]*Gender, error) {
	...
	if query := gq.withGenderID; query != nil {
		...
		query.Where(predicate.User(func(s *sql.Selector) {
			s.Where(sql.InValues(gender.GenderIDColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.gender_gender_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "gender_gender_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "gender_gender_id" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.GenderID = append(node.Edges.GenderID, n)
		}
	}
